...
public class TeleportWhitelistRemove implements ITeleportInterface {
    ...
    /*
      Command structure:
        /teleport   whitelist remove   player
     */
    @Override
    public boolean execute(CommandSender commandSender, String[] args) {
        ...
        String cmdPermission = "itscore.teleport.whitelist.remove";
        ...
        // Check arguments
        if (args.length == 2) {
            noArgsMessageLines = lang.getStringList("modules.teleport.subcommands.whitelist.remove.no-args");
        ...
        if (target.equals(player)) {
            uMessage.playerMessage(player, lang.getString("modules.teleport.subcommands.whitelist.remove.target-same-player"));
        ...
        if (!found) {
            uMessage.playerMessage(player, lang.getString("modules.teleport.subcommands.whitelist.remove.not-on-list"));

            return true;
        }

        uMessage.playerMessage(player, lang.getString("modules.teleport.subcommands.whitelist.remove.removing"));

        mTeleportManager.removeTargetFromPlayerWhitelist(player.getUniqueId().toString(), target.getUniqueId().toString());
        uMessage.playerMessage(player, lang.getString("modules.teleport.subcommands.whitelist.remove.success").replace("{name}", target.getName()));
        return true;
    }
}
